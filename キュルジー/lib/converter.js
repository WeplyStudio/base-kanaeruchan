...
async function imageToWebp(imageBuffer) {
    try {
        ...
        // Resize image to max 512x512 for sticker
        const resized = image.resize(512, 512);
        const webpBuffer = await resized.getBufferAsync(jimp.MIME_PNG);
        return webpBuffer;
    } catch (error) {
...
/**
 * Video to Webp (for animated stickers)
 * @param {Buffer} videoBuffer
 * @returns {Promise<Buffer>}
 */
async function videoToWebp(videoBuffer) {
    // For now, just return the buffer as-is
    // You can implement ffmpeg conversion here if needed
    return videoBuffer;
}
